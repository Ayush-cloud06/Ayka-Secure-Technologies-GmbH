isms/
│
├── 00-context-and-governance/
│   ├── scope.md
│   ├── context-of-organization.md
│   ├── interested-parties.md
│   ├── legal-and-regulatory-requirements.md
│   ├── information-security-policy.md
│   ├── isms-objectives.md
│   ├── management-commitment-statement.md
│   ├── document-control-procedure.md
│   ├── document-register.md
│   └── record-retention-policy.md
│
├── 01-organization/
│   ├── org-chart.md
│   ├── roles-and-responsibilities.md
│   ├── personnel-register.md
│   ├── confidentiality-agreements/
│   │   ├── template-nda.md
│   │   └── signed-nda-index.md
│   ├── competence-matrix.md
│   ├── training-and-awareness-program.md
│   ├── disciplinary-process.md
│   └── onboarding-offboarding-procedure.md
│
├── 02-asset-management/
│   ├── asset-inventory.md
│   ├── asset-classification-policy.md
│   ├── asset-ownership-register.md
│   ├── acceptable-use-policy.md
│   ├── remote-work-policy.md
│   ├── mobile-device-policy.md
│   └── asset-return-checklist.md
│
├── 03-risk-management/
│   ├── risk-management-methodology.md
│   ├── risk-register.md
    ├── risk-criteria.md
│   ├── risk-treatment-plan.md
│   ├── risk-acceptance-log.md
│   ├── risk-assessment-results/
│   │   ├── 2026-q1-risk-assessment.md
│   │   └── 2026-q2-risk-review.md
│   └── threat-scenarios-catalog.md
│
├── 04-controls-and-soa/
│   ├── statement-of-applicability.md
│   ├── control-mapping-to-internal-it.md
│   ├── control-gap-analysis.md
│   └── justification-for-exclusions.md
│
├── 05-operational-policies/
│   ├── incident-response-policy.md
│   ├── access-control-policy.md
│   ├── password-policy.md
│   ├── mfa-policy.md
│   ├── privileged-access-policy.md
│   ├── logging-and-monitoring-policy.md
│   ├── cloud-security-policy.md
│   ├── secure-development-policy.md
│   ├── change-management-policy.md
│   ├── backup-policy.md
│   ├── encryption-policy.md
│   ├── vulnerability-management-policy.md
│   ├── patch-management-policy.md
│   ├── supplier-security-policy.md
│   ├── data-protection-policy.md
│   ├── data-retention-policy.md
│   └── clean-desk-and-clear-screen-policy.md
│
├── 06-operational-procedures/
│   ├── user-access-procedure.md
│   ├── access-review-procedure.md
│   ├── incident-response-procedure.md
│   ├── backup-and-restore-procedure.md
│   ├── change-management-procedure.md
│   ├── vulnerability-scanning-procedure.md
│   ├── patch-deployment-procedure.md
│   ├── supplier-onboarding-procedure.md
│   ├── data-breach-notification-procedure.md
│   ├── business-continuity-procedure.md
│   ├── disaster-recovery-procedure.md
│   └── corrective-action-procedure.md
│
├── 07-technical-evidence/
│   ├── architecture-diagrams/
│   │   ├── network-architecture.png
│   │   ├── cloud-organization-structure.png
│   │   └── data-flow-diagram.png
│   ├── internal-it-mapping.md
│   ├── terraform-module-mapping.md
│   ├── logging-evidence/
│   ├── access-review-logs/
│   ├── backup-test-results/
│   ├── vulnerability-scan-reports/
│   └── penetration-test-report.md
    └── control-evidence-index.md
│
├── 08-monitoring-and-measurement/
│   ├── kpi-and-metrics.md
│   ├── security-metrics-dashboard.md
│   ├── monitoring-results/
│   │   ├── q1-monitoring.md
│   │   └── q2-monitoring.md
│   └── supplier-performance-review.md
│
├── 09-internal-audit/
│   ├── audit-programme.md
│   ├── audit-plan-2026.md
│   ├── audit-checklists/
│   ├── audit-reports/
│   └── audit-findings-log.md
│
├── 10-management-review/
│   ├── management-review-agenda.md
│   ├── management-review-minutes-q1.md
│   ├── management-review-minutes-q2.md
│   └── management-review-action-items.md
│
├── 11-improvement/
│   ├── nonconformity-log.md
│   ├── corrective-action-log.md
│   ├── improvement-register.md
│   └── lessons-learned-log.md
│
└── README.md